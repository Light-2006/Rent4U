  ...
  return (
    ...
    >
      <div className="grid grid-cols-1 gap-3">
        <input value={form.name} onChange={(e) => handle('name', e.target.value)} placeholder="Tên sản phẩm" className="px-3 py-2 border rounded" />
        <input value={form.provider} onChange={(e) => handle('provider', e.target.value)} placeholder="Nhà cung cấp" className="px-3 py-2 border rounded" />
        <input value={form.pricePerDay} onChange={(e) => handle('pricePerDay', e.target.value)} placeholder="Giá / ngày" className="px-3 py-2 border rounded" />
        <input value={form.category} onChange={(e) => handle('category', e.target.value)} placeholder="Danh mục" className="px-3 py-2 border rounded" />
        <textarea value={form.description} onChange={(e) => handle('description', e.target.value)} placeholder="Mô tả" className="px-3 py-2 border rounded" />
        <input value={form.images} onChange={(e) => handle('images', e.target.value)} placeholder="Images (comma separated)" className="px-3 py-2 border rounded" />
        <input value={form.sizes} onChange={(e) => handle('sizes', e.target.value)} placeholder="Sizes (comma)" className="px-3 py-2 border rounded" />
        <input value={form.colors} onChange={(e) => handle('colors', e.target.value)} placeholder="Colors (name:hex, comma)" className="px-3 py-2 border rounded" />
        ...
        <div className="flex gap-2">
          <button className="px-4 py-2 bg-[#8B6F47] text-white rounded">Lưu</button>
      ...
      <Shell>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-24">
          <div className="flex flex-col items-center gap-4">
            ...
            <h1 className="font-display text-2xl text-[#3D2B1F]">Chỉ dành cho quản trị viên</h1>
            <p className="text-sm text-[#9B8E84] max-w-md text-center">Vui lòng đăng nhập bằng tài khoản quản trị để truy cập trang này.</p>
            <div className="flex gap-3 mt-4">
              <Link to="/login" className="px-6 py-2.5 bg-[#8B6F47] text-white rounded-xl">Đăng nhập</Link>
              <Link to="/" className="px-6 py-2.5 border border-[#C4A882] text-[#8B6F47] rounded-xl">Về trang chủ</Link>
            </div>
          </div>
        </div>
      </Shell>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="flex items-center gap-3">
            <div className="flex rounded-lg overflow-hidden border">
              <button onClick={() => setActiveTab('products')} className={`px-4 py-2 ${activeTab === 'products' ? 'bg-[#8B6F47] text-white' : 'bg-white'}`}>Products</button>
              <button onClick={() => setActiveTab('users')} className={`px-4 py-2 ${activeTab === 'users' ? 'bg-[#8B6F47] text-white' : 'bg-white'}`}>Users</button>
            </div>
        ...
        {activeTab === 'products' ? (
              ...
              {products.map((p) => (
                <div key={p.id} className="bg-white border rounded p-3">
                  <div className="flex items-start gap-3">
                    <div className="w-20 h-28 bg-gray-100 flex-shrink-0 rounded overflow-hidden">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-xs text-[#9B8E84]">{p.provider}</p>
                      <p className="text-sm text-[#8B6F47] mt-2">Giá: {p.pricePerDay.toLocaleString()} / ngày</p>
                      <div className="flex gap-2 mt-3">
                        <button onClick={() => setEditing(p.id)} className="px-3 py-1 border rounded text-sm">Sửa</button>
                        <button onClick={() => { if (confirm('Xoá sản phẩm?')) { deleteProduct(p.id); toast.success('Đã xoá sản phẩm'); } }} className="px-3 py-1 border rounded text-sm text-red-500">Xoá</button>
                      </div>
                    </div>
                  </div>
          ...
          <div className="bg-white p-4 rounded border">
            ...
            {users.length === 0 ? (
              <div className="py-8 text-center text-sm text-[#9B8E84]">Chưa có người dùng nào.</div>
            ) : (
                ...
                {users.map((u) => (
                  <div key={u.id} className="flex items-center justify-between p-3 border rounded">
                    <div>
                      ...
                      <p className="text-xs text-[#9B8E84]">{u.email}</p>
                      <p className="text-xs text-[#6B5135] mt-1">Role: <strong className="text-[#8B6F47]">{u.role}</strong></p>
                    </div>
