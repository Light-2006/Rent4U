  ...
  return (
    ...
    >
      <div className="grid grid-cols-1 gap-3">
        ...
        <input value={form.provider} onChange={(e) => handle('provider', e.target.value)} placeholder="Nhà cung cấp (hiển thị)" className="px-3 py-2 border rounded" />
      ...
      <Shell>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-24">
          <div className="flex flex-col items-center gap-4">
            ...
            <h1 className="font-display text-2xl text-[#3D2B1F]">Bạn cần đăng nhập</h1>
            <p className="text-sm text-[#9B8E84] max-w-md text-center">Vui lòng đăng nhập để truy cập trang chủ shop của bạn.</p>
    ...
    return (
      <Shell>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-24">
          <div className="flex flex-col items-center gap-4">
            <div className="w-24 h-24 bg-[#F0E8DC] rounded-full flex items-center justify-center text-4xl">🔑</div>
            <h1 className="font-display text-2xl text-[#3D2B1F]">Trang chủ shop</h1>
            <p className="text-sm text-[#9B8E84] max-w-md text-center">Bạn chưa đăng ký là chủ shop. Nhấn nút bên dưới để đăng ký và bắt đầu đăng sản phẩm.</p>
            <div className="flex gap-3 mt-4">
              <button
                onClick={() => { setShopOwner(true); toast.success('Bạn đã đăng ký chủ shop'); navigate('/shopowner'); }}
                className="px-6 py-2.5 bg-[#8B6F47] text-white rounded-xl"
              >
                Trở thành chủ shop
              </button>
              <Link to="/profile" className="px-6 py-2.5 border border-[#C4A882] text-[#8B6F47] rounded-xl">Trang cá nhân</Link>
            </div>
          </div>
        </div>
      </Shell>
    ...
    <Shell>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="flex gap-2">
            <button onClick={() => setShowNew((s) => !s)} className="px-4 py-2 bg-[#8B6F47] text-white rounded">Đăng sản phẩm mới</button>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {ownerProducts.length === 0 && (
            <div className="col-span-full bg-white border rounded p-6 text-center">Bạn chưa có sản phẩm nào. Hãy đăng sản phẩm đầu tiên!</div>
          )}
